...
func ParseTime(input string) (Time, error) {
	...
	if len(components) != 3 {
		return Time{}, &TimeParseError{"Invalid number of timecomponents", input}
	} else {
		...
		if hour > 23 || hour < 0 {
			return Time{}, &TimeParseError{"hour not in valid range", fmt.Sprintf("%v", hour)}
		}
		if minute > 59 || minute < 0 {
			return Time{}, &TimeParseError{"minute not in valid range", fmt.Sprintf("%v", minute)}
		}
		if second > 59 || second < 0 {
			return Time{}, &TimeParseError{"second not in valid range", fmt.Sprintf("%v", second)}
		}
